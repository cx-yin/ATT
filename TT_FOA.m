...
S_re{1} = 100*eye(tt_rank(1));
S_re{2} = 100*eye(tt_rank(1)*tt_rank(2));
S_re{3} = 100*eye(tt_rank(2)*tt_rank(3));
...
for ii = 1 : T
    ...
    idx       = randperm(length(xt),10*tt_rank(3));
    g4        = H(idx,:) \ xt(idx);
    G4        = [G4,g4];
    ...
    G234      = G234(:,:,:,1);
    G234_mat  = ten2mat(G234,1);
    
    S_re{1}  = G234_mat * G234_mat' + lambda*S_re{1};
    G_inv = S_re{1} \ G234_mat;
